...
facefusion.globals.config_path = 'facefusion/facefusion.ini'
facefusion.globals.skip_download=True
import signal
import onnxruntime

from facefusion import core,wording,metadata,config
...
def cli_api()->None:
	...
	group_misc.add_argument('--log-level', help = wording.get('help.log_level'), default = config.get_str_value('misc.log_level', 'info'), choices = core.logger.get_log_levels())
	# execution
	...
	core.logger.init(facefusion.globals.log_level)

	if facefusion.globals.system_memory_limit > 0:
		core.limit_system_memory(facefusion.globals.system_memory_limit)
	# if facefusion.globals.force_download:
	# 	core.force_download()
	# 	return
	if not core.pre_check() or not core.content_analyser.pre_check() or not core.face_analyser.pre_check() or not core.face_masker.pre_check() or not core.voice_extractor.pre_check():
		return
...
def swap_enchnacer_face(sourcePath,targetPath,outputPath):
    face_swapper.process_image([sourcePath],targetPath,outputPath)
    face_swapper.post_process()
    face_enhancer.process_image([sourcePath],outputPath,outputPath)
    face_swapper.post_process()

cli_api()
facefusion.globals.face_selector_mode = "one"

# swap_enchnacer_face("source.jpg","target.png","result.png")